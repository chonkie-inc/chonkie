...
# Initialize the handshake
handshake = ChromaHandshake(collection_name="my_documents")
embedding = handshake.embedding_function.embedding_model.embed("best chonker").tolist()
...
# Create some chunks
chunker = SemanticChunker(embedding_model=handshake.embedding_function.embedding_model)
chunks = chunker.chunk("Chonkie is the best chonker ever!")

# Search the handshake using an embedding from a chunk
