...
<Callout type="info">For installation instructions, see the [Installation Guide](/chonkie/installation).</Callout>
...
SemanticChunker supports multiple embedding providers through Chonkie's embedding system. See the [Embeddings Overview](/chonkie/embeddings/overview) for more information.
