...
<Tabs items={["in-memory (ephemeral)", "local persistent storage", "existing connection", "LanceDB Cloud"]}>
<Tab value="in-memory (ephemeral)">
```python
...
<Tab value="local persistent storage">
```python
from chonkie import LanceDBHandshake
...
<Tabs items={["search using a query", "search using an embedding"]}>
...
<Tab value="search using an embedding">
```python
from chonkie import LanceDBHandshake
