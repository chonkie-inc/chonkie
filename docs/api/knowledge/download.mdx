...
<ParamField path="file_id" type="string" required>
  The file key from [Get Knowledge Base Files](/api-reference/knowledge/get-files). Format: `{timestamp}-{filename}`
</ParamField>
...
file_id = "1705320000000-api-reference.pdf"
...
# Save to file
with open("downloaded-api-reference.pdf", "wb") as f:
...
const fileId = '1705320000000-api-reference.pdf';
...
curl -X GET \
  https://labs.chonkie.ai/api/v1/knowledge/product-documentation/1705320000000-api-reference.pdf \
...
# disposition = 'attachment; filename="api-reference.pdf"'
...
<Tip>
  Use streaming for large files to avoid loading entire files into memory. Most HTTP libraries support streaming downloads.
</Tip>

<Tip>
  Respect the ETag header for caching. If you've downloaded a file before, send the ETag in an If-None-Match header to check if it's changed.
</Tip>

<Info>
  Downloaded files are identical to the originally uploaded files. No processing or modification occurs during storage.
</Info>

<Warning>
  Very large files may take time to download. Implement timeout handling and retry logic for production applications.
</Warning>
...
The `file_id` parameter uses the format returned by [Get Knowledge Base Files](/api-reference/knowledge/get-files):

- **Format**: `{timestamp}-{filename}`
- **Example**: `1705320000000-api-reference.pdf`
