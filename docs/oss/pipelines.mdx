...
For high-throughput applications (e.g., web servers, batch processing), use `run_async()`:
...
async def process_docs():
    ...
    doc = await pipe.run_async(texts="Async processing is fast!")

    # Process multiple concurrently
    docs = await pipe.run_async(texts=["Doc 1", "Doc 2"])
