...
```
Fetcher → Vision (Optional) → Chef → Chunker → Refinery → Porter/Handshake
```
...
Extract text from scanned documents and process them:

```python
# OCR pipeline for scanned PDFs
doc = (Pipeline()
    .fetch_from("file", path="scanned_report.pdf")
    .see_with("mistral")
...
<CardGroup cols={2}>
  ...
  <Card title="Vision" icon="eye" href="/oss/vision/overview">
    Extract text from images and PDFs using OCR
  </Card>
