...
Computes and attaches OpenAI embeddings to each chunk. Each chunk in the response gains an `embedding` field containing a list of floats.

<Warning>
  Requires `OPENAI_API_KEY` to be set and the `openai` extra: `pip install "chonkie[openai]"`.
</Warning>
...
<AccordionGroup>
  <Accordion title="Parameters">
    ...
    <ParamField body="embedding_model" type="string" default='"text-embedding-3-small"'>
      OpenAI embedding model. Supported: `"text-embedding-3-small"`, `"text-embedding-3-large"`.
    </ParamField>
  </Accordion>
</AccordionGroup>
