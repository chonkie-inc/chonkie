...
# Initialize with justified overlap to add context on both sides
overlap_refinery_justified = OverlapRefinery(
    tokenizer="character",
    context_size=20,
    method="justified",                # Split overlap budget across both sides
    merge=True
)
...
<ParamField path="method" type='Literal["suffix", "prefix", "justified"]' default='"suffix"'>
  ...
  chunk to the beginning of the current chunk. `"justified"` splits the overlap
  budget across both sides so each chunk can receive context from both its
...
<ParamField path="merge" type="bool" default="True">
  ...
  appended (for `suffix`) to the `chunk.text`. For `"justified"`, the prefix
  context is prepended and the suffix context is appended. If `False`, the
  combined context is stored in `chunk.context` attribute without modifying
  `chunk.text`.
</ParamField>
