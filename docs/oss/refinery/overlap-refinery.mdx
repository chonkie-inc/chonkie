...
To use the `OverlapRefinery` via the API, check out the [API reference documentation](../../api/overlap-refinery).
...
<ParamField
  ...
  default='"character"'
>
  The tokenizer or token counter to use for calculating overlap size. Can be a
  string identifier (e.g., "character", "word", "gpt2"), a callable, or a
  `chonkie.Tokenizer` instance. Defaults to character counting.
</ParamField>
