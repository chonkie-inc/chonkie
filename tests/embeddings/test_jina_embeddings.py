...
class TestJinaEmbeddingsInitialization:
    ...
    def test_initialization_with_custom_model(self) -> None:
        ...
        embeddings = JinaEmbeddings(model="jina-embeddings-v2-base-en", api_key="test_key")
...
class TestJinaEmbeddingsAPIMocking:
    ...
    def mock_batch_embedding_response(self) -> dict[str, Any]:
        ...
        return {
            ...
            "data": [
                ...
                {"object": "embedding", "index": 2, "embedding": [0.3] * 2048},
            ],
            "model": "jina-embeddings-v4",
            "usage": {"total_tokens": 30, "prompt_tokens": 30},
        }
    ...
    def test_embed_single_text_success(
        ...
        with patch("httpx.post") as mock_post:
            mock_response = httpx.Response(
                200,
                content=json.dumps(mock_single_embedding_response).encode(),
    ...
    def test_embed_single_text_invalid_response(self, embeddings: JinaEmbeddings) -> None:
        ...
        with patch("httpx.post") as mock_post:
            mock_response = httpx.Response(
                200,
                content=json.dumps({"invalid": "response"}).encode(),
                request=httpx.Request("POST", "https://api.jina.ai/v1/embeddings"),
            )
            mock_post.return_value = mock_response
    ...
    def test_embed_batch_success(
        ...
        with patch("httpx.post") as mock_post:
            mock_response = httpx.Response(
                200,
                content=json.dumps(mock_batch_embedding_response).encode(),
                request=httpx.Request("POST", "https://api.jina.ai/v1/embeddings"),
            )
            mock_post.return_value = mock_response
            ...
            for i, result in enumerate(results):
                ...
                np.testing.assert_array_equal(
                    result,
                    np.array([expected_value] * 2048, dtype=np.float32),
                )
...
class TestJinaEmbeddingsEdgeCases:
    ...
    def test_embed_very_long_text(self, embeddings: JinaEmbeddings) -> None:
        ...
        with patch("httpx.post") as mock_post:
            mock_response = httpx.Response(
                200,
                content=json.dumps({"data": [{"embedding": [0.1] * 2048}]}).encode(),
                request=httpx.Request("POST", "https://api.jina.ai/v1/embeddings"),
            )
            mock_post.return_value = mock_response
    ...
    def test_batch_embedding_with_mixed_lengths(self, embeddings: JinaEmbeddings) -> None:
        ...
        texts = [
            ...
            "This is a very long sentence that contains many words and should test how the embedding model handles texts of varying lengths within the same batch call to ensure robust processing."
            * 5,
        ]
